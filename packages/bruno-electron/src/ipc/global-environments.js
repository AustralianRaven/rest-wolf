...
const registerGlobalEnvironmentsIpc = (mainWindow, workspaceEnvironmentsManager) => {
  ...
  ipcMain.handle('renderer:save-global-environment', async (event, { environmentUid, variables, auth, color, workspaceUid, workspacePath }) => {
    try {
      if (workspacePath && workspaceEnvironmentsManager) {
        return await workspaceEnvironmentsManager.saveGlobalEnvironmentByPath(workspacePath, { environmentUid, variables, auth, color });
      }
